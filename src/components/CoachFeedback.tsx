...
export function CoachFeedback({ feedback, targetLanguageName }: CoachFeedbackProps) {
    ...
    const { addVocabularyCard, targetLanguage, vocabulary } = useLanguageStore();
    const [isHarvested, setIsHarvested] = useState(false);

    const langCode = targetLanguage?.code || '';
    const isAlreadyInDeck = vocabulary[langCode]?.some(c => c.term.toLowerCase() === feedback.wordOfTheDay.term.toLowerCase());

    const handleHarvest = (e: React.MouseEvent) => {
        e.stopPropagation();
        addVocabularyCard({
            term: feedback.wordOfTheDay.term,
            translation: feedback.wordOfTheDay.translation,
            example: feedback.wordOfTheDay.usage,
            language: targetLanguage?.name || 'Unknown',
            category: 'Chat Discovery'
        });
        setIsHarvested(true);
    };
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex justify-between items-end">
                                    ...
                                    <button
                                        ...
                                        disabled={isHarvested || isAlreadyInDeck}
                                        className={cn(
                                            "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all",
                                            isHarvested || isAlreadyInDeck
